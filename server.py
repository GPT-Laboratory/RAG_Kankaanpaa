# simple web frontend for file-search.py
# Setup:
#
# 1. Start python in correct folder and check python version
#   C:\>cd \Temp\svn_otula_2021\turva_repository\temotero_sorsat\kankaanpaa-roadshow-demo\
#   python --version
# 
# 2. Create a virtual environment: 
#    (this is not needed for every time, steps 3 and4 are enough)
#     python -m venv venv
#
...
<html>
<head>
    <title>RoboAI-roadshow - Temotero demo - Kankaanpään kaupungin pöytäkirjat</title>
    <style>
        #query {
            width: 100%;
        }
        ...
        #header-bar img {
            height: 100%; /* Set the height of the images to fill the header bar */
...
<body>
    ...
    <h1>RoboAI-roadshow-demo (TEMOTERO): Kankaanpään kaupungin pöytäkirjat - kaupunginvaltuusto ja kaupunginhallitus</h1>
    <p>Tässä demossa havainnollistetaan dokumenttien sisältöhakua GPT4o-mallin ja vektoroitujen aineistojen kanssa. Käytämme esimerkissä julkisesti saatavilla olevaa aineistoa, mutta vastaavan toiminnan voisi yhtälailla toteuttaa yrityksen omasta aineistostaa ja yrityksen omassa suljetussa sisäverkkossa.</p>
    <form id="query-form">
        <label for="query">Syötä kysymys ja odota vastausta:</label><br><br>
                ...
                for(let i=0;i<data.citations.length;++i) {
                    ...
                    .then(data => {
                        for(let j=0;j<data.length;++j)
                            details = data[j]
                            link.innerText += ' ('+details.meeting_type + ' ' + details.meeting_date+')'
                    })
...
def get_citation_details():
    ...
    for absolute_path in absolute_paths:
        ...
        if len(parts) > 4:
            data.append({'meeting_type': parts[len(parts)-3], 'meeting_date': parts[len(parts)-2]})
    return jsonify(data)
...
if __name__ == '__main__':
    app.run(debug=False,host='0.0.0.0',port=8002) # check suitable port value
